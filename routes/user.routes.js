...
const updateValidations = [
    ...
    body('phone').optional().matches(/^\+?[\d\s-]+$/).withMessage('Número de teléfono no válido'),
    body('address.city').optional().trim().notEmpty().withMessage('La ciudad es requerida'),
    body('address.region').optional().trim().notEmpty().withMessage('La región es requerida'),
    body('address.country').optional().trim().notEmpty().withMessage('El país es requerido')
];
...
router.put('/profile', updateValidations, validateRequest, userController.updateProfile);
