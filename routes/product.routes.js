...
const productValidations = [
    body('title').trim().isLength({ min: 3, max: 150 }).withMessage('El título debe tener entre 3 y 150 caracteres'),
    ...
    body('status').optional().isIn(['disponible', 'reservado', 'vendido']).withMessage('Estado no válido')
];
...
router.patch('/:id/status',
    body('status').isIn(['disponible', 'reservado', 'vendido']).withMessage('Estado no válido'),
    validateRequest,
