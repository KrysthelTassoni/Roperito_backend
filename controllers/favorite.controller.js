    ...
    addFavorite: async (req, res) => {
        try {
            ...
            if (productCheck.rows[0].user_id === user_id) {
                return res.status(400).json({ error: 'No puedes agregar tu propio producto a favoritos' });
            }
            ...
            res.status(201).json({ message: 'Producto agregado a favoritos exitosamente' });
        } catch (error) {
    ...
    getMostFavorited: async (req, res) => {
        try {
            ...
            const query = `
                ...
                WHERE p.is_active = true AND p.status = 'disponible'
            ...
            res.status(500).json({ error: 'Error al obtener productos más favoriteados' });
        }
    }
};
