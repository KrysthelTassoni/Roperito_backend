    ...
    getOrderById: async (req, res) => {
        try {
            ...
            const query = `
                ...
                WHERE o.id = $1 AND (o.seller_id = $2 OR o.buyer_id = $2)
            `;
    ...
    updateOrderStatus: async (req, res) => {
        try {
            ...
            if (status === 'entregada') {
                await pool.query(
                    'UPDATE products SET status = $1 WHERE id = $2',
                    ['vendido', orderCheck.rows[0].product_id]
                );
            }
    ...
    confirmDelivery: async (req, res) => {
        try {
            ...
            if (orderCheck.rows[0].buyer_id !== userId) {
                return res.status(403).json({ error: 'No autorizado para confirmar esta entrega' });
            }

            if (orderCheck.rows[0].status !== 'enviada') {
                return res.status(400).json({ error: 'La orden debe estar en estado enviada para confirmar la entrega' });
            }
    ...
    cancelOrder: async (req, res) => {
        try {
            ...
            if (!['pendiente', 'confirmada'].includes(order.status)) {
                return res.status(400).json({ error: 'No se puede cancelar una orden que ya ha sido enviada o entregada' });
            }
